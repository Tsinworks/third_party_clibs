...
LIBIMOBILEDEVICE_API idevice_error_t idevice_new(idevice_t * device, const char *udid)
{
	...
	if (res > 0) {
        ...
        case UD_IOS:
            ...
            dev->type = IDEVICE_IOS;
            break;
        case UD_ANDROID:
            dev->conn_type = CONNECTION_ADB;
            dev->type = IDEVICE_ANDROID;
            break;
        }
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_connect(idevice_t device, uint16_t port, idevice_connection_t *connection)
{
    ...
    if (device->conn_type == CONNECTION_USBMUXD) {
        ...
        *connection = new_connection;
        return IDEVICE_E_SUCCESS;
    } else if (device->conn_type == CONNECTION_ADB) {
        int sfd = usbmuxd_adb_connect();
        if (sfd < 0) {
            debug_info("ERROR: Connecting to usbmuxd failed: %d (%s)", sfd, strerror(-sfd));
            return IDEVICE_E_UNKNOWN_ERROR;
        }
        ...
        idevice_connection_t new_connection = (idevice_connection_t)malloc(sizeof(struct idevice_connection_private));
        new_connection->type = CONNECTION_ADB;
        new_connection->data = (void*)(long)sfd;
        new_connection->ssl_data = NULL;
        idevice_get_udid(device, &new_connection->udid);
        // set device uuid
        char buffer[128] = { 0 };
        int len = snprintf(buffer, 128, "host:transport:%s", new_connection->udid);
        adb_connection_send_request(new_connection, buffer);
        *connection = new_connection;
        return IDEVICE_E_SUCCESS;
	} else {
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_disconnect(idevice_connection_t connection)
{
	...
	if (connection->type == CONNECTION_USBMUXD || connection->type == CONNECTION_ADB) {
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_connection_send(idevice_connection_t connection, const char *data, uint32_t len, uint32_t *sent_bytes)
{
	...
	if (connection->ssl_data && connection->type == CONNECTION_USBMUXD) {
#ifdef HAVE_OPENSSL
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_get_handle(idevice_t device, uint32_t *handle)
{
	...
	if (device->conn_type == CONNECTION_USBMUXD || device->conn_type == CONNECTION_ADB) {
...
LIBIMOBILEDEVICE_API enum idevice_type idevice_get_type(idevice_t device)
{
    return device->type;
}
...
{
	if (!connection || connection->ssl_data || connection->type != CONNECTION_USBMUXD)
		return IDEVICE_E_INVALID_ARG;
...
{
	if (!connection || connection->type != CONNECTION_USBMUXD)
		return IDEVICE_E_INVALID_ARG;
	if (!connection->ssl_data) {

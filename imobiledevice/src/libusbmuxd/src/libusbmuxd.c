...
static uint16_t adb_port = ADB_SOCKET_PORT;
 ...
 */
static usbmuxd_device_info_t *devices_find_ios(uint32_t handle)
{
...
static usbmuxd_device_info_t *devices_find_android(char* udid)
{
    if (!udid) 
        return NULL;
    FOREACH(usbmuxd_device_info_t *dev, &devices) {
        if (dev && dev->type == UD_ANDROID) {
            if (!strcmp(dev->udid, udid)) {
                return dev;
            }
        }
    } ENDFOREACH
...
static usbmuxd_device_info_t *devices_find_android_inlist(char* udid, struct collection* cdevices)
{
    if (!udid)
        return NULL;
    FOREACH(usbmuxd_device_info_t *dev, cdevices) {
        if (dev && dev->type == UD_ANDROID) {
            if (!strcmp(dev->udid, udid)) {
                return dev;
            }
...
static int connect_adb_socket()
{
    return socket_connect(tcp_host, adb_port);
}

USBMUXD_API int usbmuxd_adb_connect()
{
    return connect_adb_socket();
}
 ...
 * 
 */
static int send_format_adb_request(int sfd, const char* request)
{
    int req_len = strlen(request);
    char buffer[1024] = { 0 };
    int count = snprintf(buffer, 1024, "%04x%s", req_len, request);
    return socket_send(sfd, buffer, count);
}

static int read_adb_response(int sfd, int* succeed, char** msg, int timeout)
{
    char buffer[5] = { 0 };
    int recv_len = socket_receive_timeout(sfd, buffer, 4, 0, timeout);
    if (recv_len < 4)
        return -EBADMSG;
    ...
    else
    {
        *succeed = 0;
        // read error msg
        int payload = 0;
        recv_len = socket_receive_timeout(sfd, &payload, 4, 0, timeout);
        if (recv_len < 4)
            return -EBADMSG;
        if (payload > 0)
        {
            *msg = (char*)malloc(payload + 1);
            (*msg)[payload] = '\0';
            recv_len = socket_receive_timeout(sfd, *msg, payload, 0, timeout);
            if (recv_len < payload)
                return -EBADMSG;
        }
    }
...
static int send_adb_track_devices(int sfd)
{
    int res = -1;
    res = send_format_adb_request(sfd, "host:track-devices");
    return res;
}

static int read_track_devices_response(int sfd, struct collection* device_list)
{
    int succeed = 0;
    char* err_msg = NULL;
    int ret = read_adb_response(sfd, &succeed, &err_msg, 0);
    if (succeed) {
        char buffer[4] = { 0 };
        int payload = 0;
        int recv_len = socket_receive_timeout(sfd, buffer, 4, 0, 0);
        if (recv < 4) {
            DEBUG(1, "%s: ERROR: could not read device list from adb host:track-devices\n", __func__);
            return -EBADMSG;
        }
        sscanf(buffer, "%04x", &payload);
        if (payload > 0) {
            char* resp = (char*)malloc(payload + 1);
            resp[payload] = '\0';
            recv_len = socket_receive_timeout(sfd, resp, payload, 0, 0);
            if (recv_len < payload) {
                DEBUG(1, "%s: ERROR: could not read complete device list from adb host:track-devices\n", __func__);
                return -EBADMSG;
            }
            ...
            while (line) {
                ...
                if (!strcmp(line, "device")) {
                    ...
                    devinfo->type = UD_ANDROID;
                    devinfo->handle = 1; // means android device 
                    devinfo->product_id = -1;
                    strcpy(devinfo->udid, uuid);
                    collection_add(device_list, devinfo);
                }
                line = strtok(NULL, "\n\t");
            }
            return recv_len;
    ...
    else
    {
        if (err_msg)
        {
            DEBUG(1, "%s: ERROR: could not read response from adb host:track-devices, %s\n", __func__, err_msg);
            free(err_msg);
        }
        else
        {
            DEBUG(1, "%s: ERROR: could not read response from adb host:track-devices\n", __func__);
        }
        return ret;
    }
...
static int adb_listen()
{
    int sfd;
    int res = -1;
    sfd = connect_adb_socket();
    res = send_adb_track_devices(sfd);
    return sfd;
}
 ...
 */
static int get_next_event_ios(int sfd, usbmuxd_event_cb_t callback, void *user_data)
{
	struct usbmuxd_header hdr;
	void *payload = NULL;
	...
	if (hdr.message == MESSAGE_DEVICE_ADD) {
		...
		devinfo = devices_find_ios(handle);
		if (!devinfo) {
...
static int get_next_event_android(int sfd, usbmuxd_event_cb_t callback, void *user_data)
{
    struct collection tmpdevs;
    collection_init(&tmpdevs);
    int res = read_track_devices_response(sfd, &tmpdevs);

    FOREACH(usbmuxd_device_info_t *dev, &tmpdevs) { // iterate current list
        usbmuxd_device_info_t* finddev = devices_find_android(dev->udid);
        if (!finddev) {
            generate_event(callback, dev, UE_DEVICE_ADD, user_data);
            collection_add(&devices, dev);
        }
    } ENDFOREACH

    FOREACH(usbmuxd_device_info_t *dev, &devices) {
        usbmuxd_device_info_t* finddev = devices_find_android_inlist(dev->udid, &tmpdevs);
        if (!finddev) {
            generate_event(callback, dev, UE_DEVICE_REMOVE, user_data);
            collection_remove(&devices, dev);
            free(dev);
...
static void *device_monitor(void *data)
{
	...
	while (event_cb) {
        ...
        if (adblistenfd == -1) {
            adblistenfd = adb_listen();
        }
		while (event_cb && (listenfd != -1 || adblistenfd != -1)) {
            if (listenfd == -1) {
                listenfd = usbmuxd_listen();
            }
			int res = get_next_event_ios(listenfd, event_cb, data);
			if (res < 0) {
                listenfd = -1;
			}
            if (adblistenfd == -1) {
            ...
            res = get_next_event_android(adblistenfd, event_cb, data);
            if (res < 0) {
                adblistenfd = -1;
            }
		}
...
get_ios_device:
	...
	if (sfd < 0) {
        ...
        goto get_android_device_list; // get android device
	}
...
got_ios_device_list:
	...
	socket_close(sfd);
    goto get_android_device_list;

get_android_device_list:
    sfd = connect_adb_socket();
    if (sfd < 0) {
        DEBUG(1, "%s: error opening adb socket!\n", __func__);
        goto final_collect;
    }
	if (tmpdevs.list == NULL)
		collection_init(&tmpdevs);
    int ret = send_adb_track_devices(sfd);
    read_track_devices_response(sfd, &tmpdevs);
    socket_close(sfd);

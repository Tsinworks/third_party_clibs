...
LIBIMOBILEDEVICE_API syslog_relay_error_t syslog_relay_client_new(idevice_t device, lockdownd_service_descriptor_t service, syslog_relay_client_t * client)
{
	*client = NULL;
    ...
    enum idevice_type dt = idevice_get_type(device);
    if (dt == IDEVICE_IOS) {
        if (!service || service->port == 0) {
            debug_info("Incorrect parameter passed to ios syslog_relay_client_new.");
            return SYSLOG_RELAY_E_INVALID_ARG;
        }
    ...
    if (dt == IDEVICE_ANDROID) {
        adb_client_error_t err = adb_connection_send_request(parent->connection, "shell:logcat");
        if (err != ADB_CLIENT_E_SUCCESS) {
            debug_info("failed to open Android logcat. Error: %i", err);
        }
    }
